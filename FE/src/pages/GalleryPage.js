...
const GalleryPage = () => {
  ...
  const { currentUser, isLoggedIn, loadingAuth } = useAuth(); // Lấy thông tin user từ AuthContext
  ...
  const renderContent = () => {
    // Chờ cả auth và data load xong, hoặc chỉ data nếu chưa có post
    if (loadingAuth || (loading && posts.length === 0 && !error) ) {
      return (
    ...
    if (error) {
      return (
        ...
        `}>
          ...
          <h3 className="text-xl font-semibold text-red-700 mb-2">Lỗi</h3>
    ...
    if (isContentLoaded && !loading && !error && posts.length === 0) {
        return (
            ...
            `}>
                ...
                <h3 className="text-2xl sm:text-3xl font-semibold text-slate-800 mb-3">Thư viện trống</h3>
                <p className="text-md sm:text-lg text-gray-600 mb-8">
                  Chưa có khoảnh khắc nào được chia sẻ.
                  {isLoggedIn && " Hãy đăng bài đầu tiên!"} {/* Hiển thị nếu đã đăng nhập */}
                </p>
                {/* Chỉ hiển thị nút Post nếu đã đăng nhập và auth không loading */}
    ...
    <div className="bg-gradient-to-br from-slate-100 via-gray-100 to-slate-200 min-h-screen py-10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <header className="flex flex-col sm:flex-row items-center justify-between mb-10 sm:mb-12 pb-6 border-b border-slate-200">
          ...
          {/* Chỉ hiển thị nút "Đăng bài mới" nếu đã đăng nhập và auth không loading */}
          {!loadingAuth && isLoggedIn && (
              <button
                onClick={handleOpenAddModal}
      ...
      {/* AddPostModal chỉ nên được render và mở nếu đã đăng nhập */}
